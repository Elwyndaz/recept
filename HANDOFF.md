...
- **Fas 2b ägarprofiler är byggt lokalt men INTE deployat 2026-07-08.** Ägaretiketten "från X" länkar till `#/anvandare/:ownerId`; workern har `GET /users/:id/recipes`; publika receptlänkar använder `ownerId|id` för att tåla slug-krockar. `index.html` är cache-bustad till `app.js?v=ownerprofiles-20260708`. Verifierat lokalt med `node test.js`, `node --check app.js`, `node --check worker/worker.js`, `npx wrangler deploy --dry-run`.
...
5. Deploy/prodverifiera lokal fas 2b när Patrik godkänner: D1-backup först enligt rutin, deploya worker, pusha frontend, testa inloggat att "från Julia" öppnar Julias offentliga recept och att spara/ta bort fortfarande använder rätt `ownerId|id`.
