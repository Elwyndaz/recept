...
- **Fas 2b ägarprofiler är deployad 2026-07-08.** Worker-version `14cfc95f-ee69-4d49-8fc9-b50c68b4466a`, frontend-commit `74140f9`. D1-backup togs först: `backups/recept-2026-07-08-180527.sql`. Ägaretiketten "från X" länkar till `#/anvandare/:ownerId`; workern har `GET /users/:id/recipes`; publika receptlänkar använder `ownerId|id` för att tåla slug-krockar. `index.html` är cache-bustad till `app.js?v=ownerprofiles-20260708`. Verifierat med `node test.js`, `node --check app.js`, `node --check worker/worker.js`, `npx wrangler deploy --dry-run`; prod read-only: nya endpointen ger 401 med ogiltig token och cache-bustad `index.html` laddar nya app.js.
...
5. Prodverifiera fas 2b inloggat: tryck på "från Julia" och kontrollera att Julias offentliga recept visas; testa också att spara/ta bort fortfarande använder rätt `ownerId|id`.
