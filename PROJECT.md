...
- `nutrients.json`: global uppslagstabell ingrediensnamn (samma nyckel som `keyOf` i app.js) → kcal/protein/kolhydrater/fett per 100 g/ml, baserad på generella publicerade näringsvärden (Livsmedelsverkets API kräver oklar autentisering och gick inte att nå, se `research`-notiser om det blir aktuellt igen). `nutritionPerPortion(recipe, nutrients)` i app.js räknar ut värde per portion direkt från receptets grundmängder (oberoende av vald portionsskala), `ml` behandlas som `g`. Ingrediens som saknas i filen räknas inte med, visas som "ofullständigt" i receptvyn. Växer manuellt vid nya ingredienser, samma underhållsmönster som nya startrecept.
