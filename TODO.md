...
- [x] **Fas 3 grupper deployad 2026-07-08 kväll** (worker `b0a8676b`, backup `recept-2026-07-08-203216.sql` togs först). Innehåller `groups`, `group_members`, `invites`, `GET /friends-feed`, `GET/POST /groups`, `POST /groups/:id/invite`, `POST /join/:code`, fliken `#/vanner`, join-route `#/join/:code` och gruppsektion under Konto. Lokalt API-smoketestat hela flödet; prod read-only 401-verifierat. Kvar: inloggat prodtest (skapa grupp, inbjudan, join, `#/vanner`).
